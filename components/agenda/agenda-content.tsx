...
export function AgendaContent({ openDialog, onDialogChange }: AgendaContentProps) {
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col gap-4">
      ...
      >
        <DialogContent className="flex max-h-[calc(100dvh-1rem)] min-w-0 flex-col gap-0 overflow-hidden p-0 max-sm:left-0 max-sm:top-0 max-sm:h-[100dvh] max-sm:max-h-none max-sm:max-w-none max-sm:translate-x-0 max-sm:translate-y-0 max-sm:rounded-none max-sm:border-0 max-sm:[&_[data-slot=dialog-close]]:right-5 max-sm:[&_[data-slot=dialog-close]]:top-[calc(env(safe-area-inset-top)+1rem)] sm:h-[min(40rem,calc(100dvh-1rem))] sm:max-w-xl">
          <Button
            type="button"
            variant="ghost"
            size="sm"
