...
test("exibe vencimentos e valor de renovação dos próximos seis meses", async ({ page }) => {
  ...
  const threeMonthOption = page.getByRole("menuitemradio", { name: "3 meses" })
  await expect(threeMonthOption).toBeVisible()
  await threeMonthOption.click()
  await expect(threeMonthOption).toHaveCount(0)
  ...
  await periodPicker.click()
  const oneMonthOption = page.getByRole("menuitemradio", { name: "1 mês" })
  await expect(oneMonthOption).toBeVisible()
